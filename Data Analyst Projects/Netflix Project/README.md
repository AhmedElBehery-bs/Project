...
\- Added a new \*\*`imdb\\\\\\\_rating`\*\* column and Filled it (originally missing from the dataset)
...
\- `data/netflix\\\\\\\_titles\\\\\\\_enriched.csv` – Enriched catalog dataset (with added IMDb ratings and directors)

\- `data/netflix\\\\\\\_viewer\\\\\\\_data.csv` – Simulated viewer behavior dataset (500K rows, generated via Python)

\- `assets/` – Custom Netflix-style logo and icons

\- `enrichment\\\\\\\_script.py` – Python (Pandas) code for data enrichment and viewer data generation
